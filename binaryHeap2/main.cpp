...
class BinaryHeap {
  ...
  explicit BinaryHeap(Comp comp) { lessThan = comp; }
  ...
  const T & min() const
  {
      return heap.empty() ? throw std::out_of_range("Heap empty") : heap.front().value;
  }
  ...
  void bubbleUp (int index)
  {
    while(index != 0)
    {
        auto parent = TestHelper::parent(index);

        // if child is not less than parent we can end
        if(!lessThan(heap[index].value, heap[parent].value))
            break;

        // else we swap them and continue for the parent
  ...
  {
    auto l = TestHelper::child(index, 0);
    auto r = TestHelper::child(index, 1);

    auto smallerChild = index;

    if(l < size() && lessThan(heap[l].value, heap[smallerChild].value))
        smallerChild = l;

    if(r < size() && lessThan(heap[r].value, heap[smallerChild].value))
        smallerChild = r;
  ...
  void bubbleDown(int index)
  {
    while(true)
    {
        // get the smaller child of the two
  ...
  T extract_min()
  {
        ...
        Node min = std::move(heap.front());
        heap.front() = std::move(heap.back());
        heap.pop_back();
        bubbleDown(0);
        return std::move(min.value);
  }
  ...
  Ref push(T val)
  {
      heap.emplace_back(std::move(val));
      Ref ref(heap.back());
      bubbleUp(size() - 1);
      return ref;
  }
  ...
  void change(const Ref& ref, Fun f)
  {
      f(const_cast<T&>(ref.operator*()));
      fixHeap(ref_to_index(ref));
  }
  ...
  T erase(const Ref& ref)
  {
      int index = ref_to_index(ref);

      // get the value of node to be deleted
      T val = std::move(const_cast<T&>(ref.operator*()));

      // move the end value to the node referenced by ref
      const_cast<T&>(ref.operator*()) = std::move(heap.back().value);

      // swap the link to ref and the last node
      heap.back().get()->swap(*ref.get()->get());
  ...
  struct TestHelper {
    static const T& index_to_value(const BinaryHeap& H, size_t index) { return H.heap[index].value; }
    static size_t root_index() { return 0; }
    static size_t parent(size_t index) { return (index-1) / 2; }
    static size_t child(size_t parent, size_t ith) { return 2 * parent + ith + 1; }
  };
