...
bool topsortDFS(Vertex s, std::vector<bool> & visited, std::vector<Vertex> & pathStack, std::vector<bool> & isInPathStack, std::deque<Vertex> & result, const Graph & G)
{
    // stack to simulate the recursion, vertex and bool if it's ready to be closed or no
    std::vector<std::pair<Vertex, bool>> stack({{s,false}});

    while(!stack.empty())
    {
        auto [v, canClose] = stack.back(); stack.pop_back();

        // vertex has no neighbours to process, so it can be closed
        if(canClose)
        {
            pathStack.pop_back();
            isInPathStack[v] = false;
            result.push_front(v);
            continue;
        }
        // we add all neighbours of vertex in for loop to the stack, then we add all neighbours of that first neighbour etc.
        // so it can happen that vertex on the beginning of the stack was already processed as neighbour of other vertex at the end
        if(visited[v]) continue;

        // store current node before its neighbours, so we know that it can be closed after they are processed
        stack.emplace_back(v, true);

        // current note was now visited, and it's added to pathStack for cycle recreation
        visited[v] = isInPathStack[v] = true;
        pathStack.push_back(v);

        // check all neighbours if they were not visited add them into stack, if they are in path stack (currently open)
        // it means we found cycle, we recreate path from path stack and return false
        for(const auto & n : G[v])
            if(!visited[n])
                stack.emplace_back(n, false);
            else if(isInPathStack[n])
            {
                // found a loop
                int j = static_cast<int>(pathStack.size() - 1);
                while(pathStack[j--] != n);
                pathStack.erase(pathStack.begin(), pathStack.begin() + j + 1);
                return false;
            }
    }
    return true;
...
std::pair<bool, std::vector<Vertex>> topsort(const Graph& G)
{
    ...
    std::vector<bool> inPathStack(G.vertices(), false);
    std::vector<Vertex> pathStack;
