...
struct BinaryHeap {
  ...
  const T& min() const
  {
      return heap.empty() ? throw std::out_of_range("Heap empty") : heap.front();
  }
  ...
  void bubbleUp (int index)
  {
        while(index != 0)
        {
            ...
            if(!lessThan(heap[index], heap[parent]))
                break;
  ...
  int getSmallerChild(int index)
  {
        ...
        if(l < size() && lessThan(heap[l], heap[smallerChild]))
            smallerChild = l;

        if(r < size() && lessThan(heap[r], heap[smallerChild]))
          smallerChild = r;
  ...
  struct TestHelper {
    static const T& index_to_value(const BinaryHeap& H, size_t index) { return H.heap[index]; }
